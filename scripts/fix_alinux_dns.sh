...
# 阿里云 Linux 系统 DNS 工具快速修复脚本
# 专门针对阿里云 Linux 2/3 系统优化
...
echo "🔧 阿里云 Linux DNS 工具快速修复"
echo "================================="

# 检查是否为 root 用户
if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}❌ 错误: 此脚本需要 root 权限${NC}"
    echo "请使用: sudo bash scripts/fix_alinux_dns.sh"
    exit 1
fi
...
if command -v dig &> /dev/null; then
    echo "测试 dig 查询..."
    if dig +short aliyun.com &> /dev/null; then
...
# 阿里云 Linux 特殊优化
echo ""
echo -e "${BLUE}🔧 阿里云 Linux 特殊优化...${NC}"

# 检查并优化 DNS 配置
if [ -f /etc/resolv.conf ]; then
    echo "检查 DNS 配置..."
    if grep -q "nameserver 8.8.8.8" /etc/resolv.conf; then
        echo -e "${GREEN}✅ DNS 配置已优化${NC}"
    else
        echo "添加备用 DNS 服务器..."
        echo "nameserver 8.8.8.8" >> /etc/resolv.conf
        echo -e "${GREEN}✅ 已添加备用 DNS 服务器${NC}"
    fi
fi

echo ""
echo -e "${GREEN}🎉 阿里云 Linux DNS 工具修复完成！${NC}"
echo ""
echo -e "${BLUE}💡 下一步:${NC}"
